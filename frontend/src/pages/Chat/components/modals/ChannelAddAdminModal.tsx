...
interface IChannelAddAdminModalProps {
  isOpen: boolean;
  onClose: () => void;
  socket: Socket | null;
...
function ChannelAddAdminModal({
  isOpen,
  onClose,
  socket,
  currentChannel,
}: IChannelAddAdminModalProps) {
  ...
  useEffect(() => {
    if (isOpen && currentChannel) {
      // Get all members available to be promoted to admin
      const filteredMembers = currentChannel.members.filter(
        (member) =>
          !currentChannel.admins.some((admin) => admin.userId === member.id) &&
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    if (currentChannel) {
      // Request to add user as admin to channel
      fetch(`http://${localhost}:3333/chat/channel/${currentChannel.id}/admins`, {
        method: "POST",
        headers: {
        ...
        .catch((error) => {
          if (error.message !== "Unauthorized") {
            console.error(
              "An error occurred while adding the user to the admins of the channel:",
              error
            );
  ...
  return (
    ...
    >
      <h2>Promote a member of the channel to administrator</h2>
      <form
        name="add_admin"

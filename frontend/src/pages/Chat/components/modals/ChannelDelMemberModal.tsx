...
interface IChannelDelMemberModalProps {
  isOpen: boolean;
  onClose: () => void;
  socket: Socket | null;
...
function ChannelDelMemberModal({
  isOpen,
  onClose,
  socket,
  currentChannel,
}: IChannelDelMemberModalProps) {
  ...
  useEffect(() => {
    if (isOpen && currentChannel) {
      // Get all members available to be removed
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    if (!selectedUser) {
      toast.error("Please select a user to remove from the channel", {
      ...
      fetch(`http://${localhost}:3333/chat/channel/${currentChannel.id}/members`, {
        ...
        .catch((error) => {
          if (error.message !== "Unauthorized") {
            console.error(
              "An error occurred while removing the user from the channel:",
              error
            );
  ...
  return (
    ...
    >
      <h2>Remove a member from the channel</h2>
      <form name="delete_member" onSubmit={handleSubmit} className={styles["modal-form"]}>
        <select
        name="delete_member"
          value={selectedUser}
...
export default ChannelDelMemberModal;
